import { connectedToSolace } from './store/store';
import { solaceConfig } from '../config/config';
import solace, {  MessageConsumer,  MessageDeliveryModeType,  Session,  SessionEvent  } from 'solclientjs';
...
export class SolaceClient {
   ...
   */
  consumeFromQueue(queueName: string, callback: any) {
    if (this.session == null) {
  ...
  stopConsumeFromQueue(queueName: string) {
   ...
   sendRequest(topic: string, payload: string, deliveryMode: MessageDeliveryModeType, callback: any) {
      ...
      binaryAttachment.then(buffer => {
        ...
        try {
          this.session.sendRequest(message, 2000, (session: Session, msg: solace.Message) => {
            const blob = new Blob([msg.getBinaryAttachment()], { type: 'text/plain; charset=utf-8' });
            blob.text().then((text) => {
              callback(text);
          }), (session: Session, error: solace.RequestError) => {
            log.error(error);
          };
          });
   ...
   * @param deliveryMode Delivery Mode of the message
   */
  publishToTopic(topic: string, payload: string, deliveryMode: MessageDeliveryModeType) {
    if (!this.session) {
      log.warn("[WARNING] Cannot publish because not connected to Solace message router!");
      return;
    }
