import './game-over.svelte.css.proxy.js';
...
function get_each_context(ctx, list, i) {
	...
	child_ctx[10] = list[i].initials;
	child_ctx[11] = list[i].score;
...
function create_else_block(ctx) {
	...
	return {
		...
		p(ctx, dirty) {
			if (dirty & /*formatTime, leaderboard*/ 17) {
...
// (90:2) {#if !highScoreSubmitted}
function create_if_block(ctx) {
	let div1;
	let div0;
	let h1;
	let t1;
	let t2_value = /*$portfolioStore*/ ctx[3].cash.toLocaleString() + "";
	let t2;
	let t3;
	let center;
	let button;
	...
	return {
		c() {
			...
			t2 = text(t2_value);
			t3 = text(". There is nothing left to do right now but party... or you can submit your highscore to\n        the leaderboard!\n        ");
			center = element("center");
			button = element("button");
			button.textContent = "SUBMIT HIGH SCORE";
			...
			attr(div1, "class", "flex w-full game-over-text h-full svelte-1m9ffew");
		},
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = listen(button, "click", /*submitHighScore*/ ctx[5]);
				mounted = true;
			}
		},
		p(ctx, dirty) {
			if (dirty & /*$portfolioStore*/ 8 && t2_value !== (t2_value = /*$portfolioStore*/ ctx[3].cash.toLocaleString() + "")) set_data(t2, t2_value);
		},
		i: noop,
		o: noop,
		d(detaching) {
...
// (113:6) {#each leaderboard as { initials, score, timestamp }
function create_each_block(ctx) {
	...
	let t1;
	let div1;
	let t2_value = /*initials*/ ctx[10] + "";
	let t2;
	let t3;
	let div2;
	let t4;
	let t5_value = /*score*/ ctx[11].toLocaleString() + "";
	let t5;
	let t6;
	let div3;
	let t7_value = /*formatTime*/ ctx[4](/*timestamp*/ ctx[12]) + "";
	let t7;
	...
	return {
		...
		p(ctx, dirty) {
			if (dirty & /*leaderboard*/ 1 && t2_value !== (t2_value = /*initials*/ ctx[10] + "")) set_data(t2, t2_value);
			if (dirty & /*leaderboard*/ 1 && t5_value !== (t5_value = /*score*/ ctx[11].toLocaleString() + "")) set_data(t5, t5_value);
			if (dirty & /*leaderboard*/ 1 && t7_value !== (t7_value = /*formatTime*/ ctx[4](/*timestamp*/ ctx[12]) + "")) set_data(t7, t7_value);
		},
		d(detaching) {
...
function create_fragment(ctx) {
	...
	function select_block_type(ctx, dirty) {
		if (!/*highScoreSubmitted*/ ctx[1]) return 0;
		if (/*highScoreLoading*/ ctx[2]) return 1;
		return 2;
	}
...
function instance($$self, $$props, $$invalidate) {
	...
	let $portfolioStore;
	component_subscribe($$self, traderSessionStore, $$value => $$invalidate(6, $traderSessionStore = $$value));
	component_subscribe($$self, portfolioStore, $$value => $$invalidate(3, $portfolioStore = $$value));
	...
	function submitHighScore() {
		$$invalidate(2, highScoreLoading = true);
		$$invalidate(1, highScoreSubmitted = true);
		...
		setTimeout(
			() => {
				solaceClient.sendRequest('tkthetechie/leaderboard/request', JSON.stringify(new LeaderboardRequest($traderSessionStore.initials, 10)), solace.MessageDeliveryModeType.PERSISTENT, msg => {
					...
					$$invalidate(2, highScoreLoading = false);
				});
			},
			2000
		);
